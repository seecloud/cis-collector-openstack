...
class OSCollector:
    ...
    OSCollector collects information about OpenStack components
    and services state. Components for monitoring are listed in
    config.json configuration. If there is no  valid config.json
    - default settings from settings.py configuration will be used.

    OSCollector operates with two entities:
      - component - OpenStack component. Example: Nova, Keystone,
                    Neutron
      - service - component service. Example for keystone component:
      ...
      2. Get previous component state from cache
       \
        - if there is no previous state in cache - get previous state
          from backend
         \
          - if there is no previous state in backend - persist current
            state in backend, put current component state into cache
            as previous component state
        - return previous component state from cache
      3. Compare component services key by key and return changes
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.warning(" ".join(['Can not open DEFAULT_CONFIG_FILE.',
        ...
        if not self.config.get('components', None):
            ...
            raise osc_exception.CollectorError(
                'List with components for collecting data is empty')

        resource_factory.ResourceFactory.setup(self.config['components'])

        self.validate_configuration()

    def init_session(self):
        credentials = envutils.get_creds_from_env_vars()
        self.osclient = osclients.Common(**credentials)
        self.cisclient = cisclient.CISClient(settings.CIS_SERVER)

    def validate_configuration(self):
        os_services = [component.name for component
                       in self.osclient.keystone.services.list()]
        invalid_components = []
            ...
            if component not in os_services:
                ...
                logger.warning(" ".join(
                    ["Component {0}".format(component),
                     "from configuration is not available",
                     "in OpenStack services.",
                     "It will be removed from collector"]))
            if (component not in
                resource_factory.ResourceFactory.supported_schemas
                    and component not in invalid_components):
                invalid_components.append(component)
                logger.warning(" ".join(
                    ["Component {0}".format(component),
                     "from configuration has no proper resource",
                     "schemas description.",
                     "It will be removed from collector"]))

            if (isinstance(service_collector.ServiceCollector
                           .get_collector(component), type(lambda: 0))
                    and component not in invalid_components):
                invalid_components.append(component)
                logger.warning(" ".join(
                    ["Component {0}".format(component),
                     "has no collector instance.",
                     "It will be removed from configuration"]))

            if component not in invalid_components:
                invalid_services = collections.defaultdict(list)
                for service in self.config['components'][component]:
                    if (service not in resource_factory
                       .ResourceFactory.supported_schemas[component].keys()):
                        invalid_services[component].append(service)
                        logger.warning(" ".join(
                            ["Service {0}::{1}".format(component, service),
                             "from configuration has no proper resource",
                             "schema description.",
                             "It will be removed from collector"]))
    ...
    def collect(self):
        ...
        for component, services in self.config['components'].iteritems():
            p = threading.Thread(target=self.syncup,
        ...
        while time.time() - start <= self.config['delay'] * 2:
            ...
            logger.warning(" ".join(
                ["Timed out,",
                 "something goes wrong in collector process"]))

    def syncup(self, component, services):
        """Synchronize component services

        :param component: component name (e.g. nova, keystone, neutron)
        :param services: list of component services (e.g. for keystone:
               users, projects, roles)
        """
        current_state = None
        try:
            current_state = (service_collector
                             .ServiceCollector
                             .collect(self.osclient,
                                      component,
                                      services))
        ...
        if not last_state and current_state:
            for service in current_state.keys():
                logger.info(
                    "No last state found for service {0} in component {1}."
                    .format(service, component))
                self.cisclient.save_component_state(component, service,
        ...
        for service in current_state.keys():
            if current_state[service] == settings.SERVICE_INVALID_STATE:
                self.config['components'][component].remove(service)
                logger.warning(" ".join(
                    ["Service {0} was removed from configuration,"
                     .format(service),
                     "due to invalid state"]))
            else:
                changes[service] = self.compare(current_state.get(service, {}),
                                                last_state.get(service, {}),
                                                component, service)
    ...
    def compare(self, current, last, component, service):
        ...
        for key in (set(current.viewkeys())
                    .symmetric_difference(set(last.viewkeys()))):
            if key not in last:
        ...
        for key in current.viewkeys() & last.viewkeys():
            diff = (deepdiff
                    .DeepDiff(current[key], last[key],
                              ignore_order=True,
                              exclude_paths={"root['{0}']".format(key)
                              for key in self.config.get('ignore_keys', {})}))
            if diff:
