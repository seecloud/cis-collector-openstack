...
class OSCollector:
        ...
        for component in self.config['components'].keys():
            ...
            if component not in invalid_components:
                ...
                for service in self.config['components'][component]:
                    if service not in \
                     resource_factory.ResourceFactory.supported_schemas[component].keys():
    ...
    def syncup(self, component, services):
        ...
        current_state = None
        try:
            current_state = service_collector.ServiceCollector.collect(
                                                     self.osclient,
                                                     component,
                                                     services)
    ...
    def compare(self, current, last, component, service):
        ...
        for key in current.viewkeys() & last.viewkeys():
            diff = deepdiff.DeepDiff(current[key], last[key],
                                     ignore_order=True,
                                     exclude_paths={"root['{0}']"
                                     .format(key) for key
                            in self.config.get('ignore_keys', {})})
            if diff:
